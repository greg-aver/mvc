package hiber.dao;

import hiber.model.Car;
import hiber.model.User;
import org.hibernate.SessionFactory;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Repository;

import javax.persistence.TypedQuery;
import java.util.List;

@Repository
public class UserDaoImp implements UserDao {

   @Autowired
   private SessionFactory sessionFactory;

   @Override
   public void add(User user) {
      sessionFactory.getCurrentSession().save(user);
   }

   @Override
   @SuppressWarnings("unchecked")
   public List<User> listUsers() {
      TypedQuery<User> query=sessionFactory.getCurrentSession().createQuery("from User");
      return query.getResultList();
   }

   @Override
   public User getUserById(Long id) {
      TypedQuery<User> queryUser = sessionFactory.getCurrentSession().createQuery(
              "From User where car_id = :id"
      );
      queryUser.setParameter("id", id);
      return queryUser.getSingleResult();
   }

   @Override
   public Long getCarId(Car car) {
      TypedQuery<Car> queryCar = sessionFactory.getCurrentSession().createQuery("" +
              "FROM Car where name = :name and series = :series"
      );
      queryCar.setParameter("name", car.getName());
      queryCar.setParameter("series", car.getSeries());
      return queryCar.getSingleResult().getId();
   }


}
